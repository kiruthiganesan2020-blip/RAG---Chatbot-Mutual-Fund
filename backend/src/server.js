 ...
 * Express.js server with Phase 5 security and compliance integration
 */
...
// Import Phase 5 security and compliance components
...
app.post('/api/chat', async (req, res, next) => {
  try {
    ...
    const response = await chatService.processQuery(query, req.user?.id);
    ...
    res.json({
      ...
      disclaimers: response.disclaimers.map(d => d.text),
      metadata: {
        processing_time_ms: Date.now() - startTime,
        compliance_checks: {
          sebi_compliant: response.compliance_checks?.sebi_compliant,
          amfi_compliant: response.compliance_checks?.amfi_compliant
        },
